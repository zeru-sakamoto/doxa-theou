...
- **Import script:** `scripts/import_bible.py` — standalone one-off Python (stdlib `sqlite3` only; no `requests`, no deps, no API key), not part of the Tauri/Rust build. Run manually once. Reads the source DB path and table name from `BIBLE_SOURCE_DB`/`BIBLE_SOURCE_TABLE` in `.env` (the DB isn't shipped with the source; copy `.env.example`), writes a normalized `bible.sqlite`.
- **Import method:** select the 5 versions from the source's single denormalized table; seed `books` (66 rows; static OSIS abbreviations added since the source has none), `verse_refs` (union of the versions' versifications), `translations` (with license metadata added — the source carries none), and `verse_texts`; build an FTS5 index; end with an `assert` self-check. Produces ~31,102 `verse_refs` and ~155k `verse_texts`.
...
- **Git hygiene:** the app is open source, the DB is not. `.gitignore` covers `.env`, `*.sqlite`, `*.db`, and the whole `/db/` folder. `.env` holds `BIBLE_SOURCE_DB`/`BIBLE_SOURCE_TABLE` (the local source-DB path and table name); a committed `.env.example` documents it. The import script, schema, and `.env.example` are safe to commit — only the DB data (source and generated) is excluded. No API key anymore.
