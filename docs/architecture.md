...
- **Parameterized SQL.** Every query binds parameters; no string interpolation
  ...
  (`db::fts_query`): each whitespace token is wrapped as a quoted FTS term
  (embedded quotes doubled), so punctuation like `"`, `*`, `(`, `-` is matched
  literally instead of parsed as query syntax (which previously raised an error
  the user saw).
